...
export function ChartSection() {
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      if (timeRange === "1h") {
        // Untuk live data, ambil data historis 1 jam terakhir dari Firebase + current data
        try {
          const firebaseData = await getHistoricalData(0.04); // 1 jam = 0.04 hari
          if (firebaseData.length > 0) {
            ...
            // Fallback ke current data di state
            exportData = [...data];
          }
        } catch (error) {
          console.warn("Failed to get Firebase data for 1h export, using current data:", error);
          exportData = [...data];
        }
      } else {
