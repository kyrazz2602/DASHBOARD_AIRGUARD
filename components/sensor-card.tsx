...
export function SensorCard({
  ...
  if (isLoading) {
    return (
      <Card className={cn("p-4 sm:p-6 bg-card border border-border/60 space-y-3", className)}>
  ...
  return (
    <Card
      className={cn(
        "group relative overflow-hidden p-4 sm:p-6 bg-card border border-border cursor-default select-none",
    ...
    >
      ...
      <div className="mb-3 flex items-baseline flex-wrap gap-0.5 transition-transform duration-300 group-hover:scale-[1.02] origin-left">
        <span
          className={cn(
            "text-[22px] sm:text-[28px] font-bold tabular-nums leading-none",
            styles.value,
          )}
        >
          {value.toFixed(2)}
