...
import {
  ...
  Wrench,
  Activity,
  Thermometer, // Icon baru
  Battery, // Icon baru
  Zap, // Icon baru (opsional untuk charging, tapi kita pakai Battery dulu)
} from "lucide-react";
...
interface MaintenanceWidgetProps {
  ...
  temperature?: number; // Celcius (Baru)
  batteryLevel?: number; // 0 - 100 (Baru)
}

export function MaintenanceWidget({
  filterHealth = 100,
  currentPm25 = 0,
  temperature = 28, // Default value jika data belum masuk
  batteryLevel = 100, // Default value
}: MaintenanceWidgetProps) {
  // 1. Estimasi Hari (Memoized)
  const daysRemaining = useMemo(
    () => Math.max(0, Math.ceil(filterHealth * 2)),
    [filterHealth]
  );

  // 2. Status Config (Memoized)
  ...
  const loadStatus = useMemo(() => {
    if (currentPm25 > 50)
      return { text: "Heavy Load", color: "text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-900/30" };
    if (currentPm25 > 20)
  ...
  return (
    <Card
      ...
      <div className="space-y-1.5 mb-5">
        ...
        <p className="text-[10px] text-right text-muted-foreground dark:text-muted-foreground/70 font-medium pt-1">
