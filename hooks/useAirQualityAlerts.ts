...
export function useAirQualityAlerts(sensorData: any) {
  return useMemo(() => {
    const checks = [
      ...
      { key: "voc", val: sensorData.voc, std: WHO_STANDARDS.VOC, unit: "mg/m³" },
    ];
    ...
    return {
      ...
      alert: activeAlert
        ? {
            message: `${activeAlert.key.toUpperCase()} is at ${activeAlert.val.toFixed(2)} ${activeAlert.unit} - ${getStatusLabel(activeAlert.val, activeAlert.key.toUpperCase() as any)}!`,
            type:
